...
import { salesNavigation } from '@/lib/sales-navigation';
import { RoleSwitcher } from '@/components/ui/RoleSwitcher';
import {
  LayoutDashboard, Target, TrendingUp, PoundSterling,
  Building2, Users, BarChart3, Eye,
  Star, Crosshair, FileText, Calendar, Plug,
  Sparkles, X, Menu,
} from 'lucide-react';
...
export function SalesSidebar({ activePage, onNavigate, onOpenChat, mobileOpen, onCloseMobile }: SalesSidebarProps) {
  const [minutesAgo, setMinutesAgo] = useState(3);

  useEffect(() => {
    const interval = setInterval(() => setMinutesAgo(prev => prev + 1), 60000);
    ...
    <>
      ...
      <aside className={cn(
        ...
        <div className="absolute inset-0 pointer-events-none" style={{
          background: 'radial-gradient(ellipse 120% 80% at 20% 10%, rgba(74,180,232,0.06) 0%, transparent 60%), radial-gradient(ellipse 80% 60% at 80% 90%, rgba(29,183,122,0.04) 0%, transparent 60%)'
        }} />
        ...
        <div className="flex items-center gap-2 px-6 py-[14px] text-[11px] text-white/60 border-b border-white/[0.08] relative">
          <div className="w-2 h-2 bg-good rounded-full animate-pulse-dot" style={{ boxShadow: '0 0 8px #1DB77A' }} />
          <span>Live · updated {minutesAgo} min ago</span>
        </div>

        {/* Navigation */}
        <div className="flex-1 relative">
          {salesNavigation.map(section => (
            <div key={section.label}>
              ...
              {section.items.map(item => (
                ...
                >
                  <span className="w-[18px] flex justify-center">{getIcon(item.icon)}</span>
                  <span className="truncate">{item.label}</span>
                  {item.badge !== undefined && (
                    <span className={cn(
                      'ml-auto text-[10px] font-bold px-[7px] py-[2px] rounded-[10px] font-mono flex-shrink-0',
        ...
        <button
          onClick={onOpenChat}
          className="mx-4 mb-3 flex items-center gap-2 px-4 py-[10px] rounded-lg bg-sky/[0.12] text-sky-bright hover:bg-sky/[0.2] transition-all text-[13px] font-semibold border border-sky/[0.15] relative"
        >
        ...
        <RoleSwitcher currentUser="Alicia" />
      </aside>
    </>
  );
}
